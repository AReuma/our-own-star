<template>
  <div style="height: auto; width: 100%;">
    <div v-for="(comment, index) in myPageComment" :key="index">
        ...
        >
          <div style="display: flex; flex-direction: column; align-items: start; width: 100%; margin-bottom: 20px;">
            <div style="display: flex; justify-content: space-between; width: 100%; align-items: center; padding: 0 16px;">
              ...
              <div id="removeComment" style="display: flex; justify-content: end;">
                <v-btn @click.stop="deleteContent(comment.id, comment.boardId)" icon="mdi-trash-can-outline" variant="text"></v-btn>
              </div>
            </div>
...
export default defineComponent({
  ...
  created() {
    const {artist} = this;
    this.fetchMyPageComment({artist})
  },
  methods: {
    ...mapActions(['fetchMyPageComment']),
    moveBoard(index){
      const {artist} = this;
    ...
    deleteContent(commentId, boardNum){
      const {artist} = this;
