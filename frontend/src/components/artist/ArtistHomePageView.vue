<template>
  <div id="main-div" @scroll="handleScroll">
    <div id="board-content-list" v-for="n in artistBoard" :key="n">
  ...
  methods:{
    ...
    handleScroll() {
      ...
      if (scrollPos >= 0.8 && this.artistBoardTotalPage > this.currentPage) {
        ...
        this.fetchIdolCategoryAddBoard({artist, page, nickname})
      }
  ...
  watch: {
    joinCategoryUserInfo: {
      handler(newVal) {
        ...
        if (newVal) {
          const artist = this.$route.params.artist;
          this.fetchIdolCategoryBoard({ artist, nickname: newVal.nickname });
